...
::: {.callout-important}
## The game
One of you is the *programmer*, and your blindfolded partner is the *robot*. Only **separate commands**: "forward 30 cm," "turn 90° right." No steering, no touching. Get your robot across the mat to the target.
:::
...
::: {.cardbox .coral}
That gap is **open-loop drift**: moving without checking the result. Your LEGO robot is about to do exactly the same thing.
:::
...
<div class="sim">
  <div style="display:flex;justify-content:space-between;align-items:center;">
    <button class="sim-btn teal" data-sim="startRace">🏁 Race!</button>
...
function startRace(){
  ...
  var A=SIM.el('carA'), B=SIM.el('carB'),
      c=SIM.el('raceClock'),
      lA=SIM.el('lblA'), lB=SIM.el('lblB');
...
::: {.fragment .fade-up}
::: {.cardbox .teal}
**speed = distance ÷ time**: how far you get *per second*. That's the whole formula, and you invented it.
:::
:::
...
::: {.fragment .fade-up}
::: {.cardbox .coral}
`OSError`. The code says **Port A**, but the right motor is plugged into **Port B**. The program must match the *physical* robot. Check the cable, then check the letter.
:::
:::
...
::: {.fragment .fade-up}
::: {.cardbox .purple}
Throw 1 lands short, throw 2 with the *same number* lands somewhere else?! Keep your score sheet. That wobble is **evidence**, and we need it later today. 🕵️
:::
:::
...
::: {.callout-important}
## The rules
Park your robot's nose **as close to the wall as you can without touching it**. Touch = **0**. The judge measures the gap with a ruler. **Smallest gap wins.**
:::
...
::: {.fragment .fade-up}
::: {.cardbox .coral}
`2000` = **milliseconds**. That's how long it runs.
...
<div style="display:flex;justify-content:center;align-items:flex-start;gap:90px;">
  ...
  <div style="display:flex;flex-direction:column;justify-content:center;height:230px;">
    <button class="sim-btn purple" data-sim="spinMotors">▶ Spin for 3 s</button>
  </div>
  <div style="text-align:center;">
...
function spinMotors(){
  ...
  var dA=SIM.el('discA'), dB=SIM.el('discB');
  var t0=performance.now();
  function step(now){
    var t=(now-t0)/1000; if(t>3) t=3;
    ...
    SIM.el('rotA').textContent=(aA/360).toFixed(1);
    SIM.el('rotB').textContent=(aB/360).toFixed(1);
...
<div style="display:flex;justify-content:center;align-items:flex-start;gap:60px;flex-wrap:wrap;">
  ...
  <div style="display:flex;flex-direction:column;gap:12px;justify-content:center;">
    <button class="sim-btn ghost" data-sim="setDial" data-sim-arg="0" >0°</button>
    <button class="sim-btn ghost" data-sim="setDial" data-sim-arg="90" >90° = ¼ turn</button>
    <button class="sim-btn ghost" data-sim="setDial" data-sim-arg="180">180° = ½ turn</button>
    <button class="sim-btn ghost" data-sim="setDial" data-sim-arg="270">270° = ¾ turn</button>
    <button class="sim-btn purple" data-sim="setDial" data-sim-arg="360">360° = full turn</button>
  </div>
</div>
...
(function(){
  ...
  SIM.def('setDial', function(target){
    ...
    function animate(now){
      ...
      currentDeg = deg;
      var rad = (deg-90)*Math.PI/180;
      SIM.el('dialNeedle').setAttribute('x2', (Math.sin(deg*Math.PI/180)*72).toFixed(2));
      SIM.el('dialNeedle').setAttribute('y2', (-Math.cos(deg*Math.PI/180)*72).toFixed(2));
      SIM.el('arcFill').setAttribute('d', describeArc(deg));
      SIM.el('dialDeg').textContent = Math.round(deg);
      SIM.el('dialFrac').textContent = fracLabel(Math.round(deg));
...
::: {.fragment .fade-up}
::: {.cardbox .teal}
**0, 30, 60, 90 … 360.** Ancient sky-watchers cut a full circle into **360 equal steps** called **degrees**. The idea comes from the **Babylonians** (~2000 BCE), who loved multiples of 60. The Greeks spread it, and we still use it today. 🏛️
:::
:::
...
::: {.fragment .fade-up}
::: {.cardbox .purple}
The **wheel** 🛞. Every degree the motor turns becomes distance on the ground.
:::
:::
...
<button class="sim-btn teal" data-sim="rollWheels">🛞 Roll one full turn</button>
</div>
<script>
var rollRAF=null;
function rollWheels(){
  ...
  var g1=SIM.el('wheel1G'), r1=SIM.el('wheel1R'), t1=SIM.el('trace1');
  var g2=SIM.el('wheel2G'), r2=SIM.el('wheel2R'), t2=SIM.el('trace2');
  var res1=SIM.el('res1'), res2=SIM.el('res2');
...
::: {.fragment .fade-up}
::: {.cardbox .purple}
Got *less* than 77? And your neighbor got a *different* number from the same code?! (Hint: check both battery lights. 🔋) **Commanding is not the same as getting.**
:::
:::
...
::: {.fragment .fade-up}
::: {.cardbox .teal}
**circumference = π × diameter.** Your wheel: 3.14 × 8.8 cm ≈ **27.6 cm**. For any wheel, read the diameter off the tire and multiply by π.
:::
:::

## 🧪 Bonus: discover π yourself

- Wrap a string around anything round: a cup, a tape roll, a bottle, your wheel. That's **C**.
...
::: {.cardbox .purple}
**2.83 < π < 4**, trapped with no string needed. It is a loose trap, though. How do we squeeze it tighter? **More sides →**
:::
:::
...
<div class="sim-stage">
  ...
  <div style="text-align:left;">
    <div style="display:flex;gap:10px;margin-bottom:18px;">
      <button class="sim-btn ghost piBtn" data-sim="setPiSides" data-sim-arg="4" >4</button>
      <button class="sim-btn ghost piBtn" data-sim="setPiSides" data-sim-arg="6" >6</button>
      <button class="sim-btn ghost piBtn" data-sim="setPiSides" data-sim-arg="12">12</button>
      <button class="sim-btn ghost piBtn" data-sim="setPiSides" data-sim-arg="24">24</button>
      <button class="sim-btn ghost piBtn" data-sim="setPiSides" data-sim-arg="96">96</button>
    </div>
...
(function(){
  ...
  SIM.def('setPiSides', function(n, btn){
    var r=150;
    SIM.el('piInnerPoly').setAttribute('points', piPts(n, r, -Math.PI/2));
    SIM.el('piOuterPoly').setAttribute('points', piPts(n, r/Math.cos(Math.PI/n), -Math.PI/2+Math.PI/n));
    SIM.el('piLo').textContent=(n*Math.sin(Math.PI/n)).toFixed(4);
    SIM.el('piHi').textContent=(n*Math.tan(Math.PI/n)).toFixed(4);
    SIM.el('piN').textContent=n;
    document.querySelectorAll('.piBtn').forEach(function(b){ b.classList.remove('on'); });
    if(btn) btn.classList.add('on');
  });
  SIM.run('setPiSides', 4, document.querySelector('.piBtn'));
})();
</script>
```
...
::: {.callout-important}
## The challenge
**Predict, then prove.** Measure your robot's speed, then tell the judge *exactly* how many seconds it needs to cross the **1.5 m** finish line. The judge starts the stopwatch, and the closest prediction wins. 3 attempts, best counts.
:::
...
::: {.fragment .fade-up}
Next: *Drive a Square*. You meet the **gyro**, feel it in your own hands, and watch it snap an open-loop square shut.
:::
