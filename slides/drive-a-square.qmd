...
::: {.notes}
Everyone types this once at the top and keeps it for the whole day. Raw motors only, on purpose — through D7 the students *build* the whole driver and library by hand; the polished shortcut (`DriveBase`) waits until the D8 reveal (decision #20, re-timed by #26), a reward earned piece by piece.
:::
...
<div class="sim-stage">
  ...
  <div style="text-align:left;">
    <div style="font-family:Consolas,monospace;font-size:22px;color:#1C355E;background:#eef3f8;border-radius:10px;padding:12px 20px;">
      robot turned: <span class="em-purple" id="tcRobot">0</span>&#176;<br>
      tire rolled: <span class="em-teal" id="tcArc">0.0</span> cm &#8594; <span id="tcWheel" style="font-weight:800;color:#E96852;font-size:26px;">0</span>&#176;
    </div>
    <div style="font-family:Consolas,monospace;font-size:18px;color:#1C355E;margin-top:10px;line-height:1.9;">
      <div class="fragment">rope around the circle = 3.14 &#215; <span id="tcGap" style="font-weight:700;">12.8</span> &#8776; <b><span id="tcRope">40.2</span> cm</b></div>
      <div class="fragment">one corner = rope &#247; 4 &#8776; <b style="color:#00AE8D;"><span id="tcQuarter">10.0</span> cm</b> of rolling</div>
      <div class="fragment">&#215; 13.04 <span style="opacity:0.7;">(yesterday's translator)</span> &#8594; TURN_ANGLE &#8776; <span id="tcTA" style="font-weight:800;color:#E96852;font-size:22px;">131</span>&#176;</div>
    </div>
    <div style="display:flex;gap:12px;margin-top:14px;">
      <button class="sim-btn teal" data-sim="tcTurn" data-sim-arg="90">&#9654; Turn 90&#176;</button>
      <button class="sim-btn coral" data-sim="tcTurn" data-sim-arg="180">&#9654; Turn 180&#176;</button>
      <button class="sim-btn purple" data-sim="tcReset">&#8634; reset</button>
    </div>
    <div style="font-size:19px;color:#1C355E;margin-top:14px;">
      tire gap: <b><span id="tcTrackVal">12.8</span> cm</b><br>
      <input type="range" min="9" max="16" step="0.1" value="12.8" data-sim-input="tcTrack" style="width:230px;accent-color:#1C355E;">
    </div>
  </div>
</div>
<script>
(function(){
  var theta = 0, target = 0, raf = null, track = 12.8, lastInc = 90;
  var ptsR = [], ptsL = [];
  function R(){ return track*10/2; }
  function setGeom(){
    var r = R();
    SIM.el('tcCircle').setAttribute('r', r);
    SIM.el('tcTireR').setAttribute('x', r-6);
    SIM.el('tcTireL').setAttribute('x', -r-6);
    SIM.el('tcBody').setAttribute('x', -(r-16));
    SIM.el('tcBody').setAttribute('width', 2*(r-16));
  }
  function draw(){
    SIM.el('tcBot').setAttribute('transform','rotate('+theta.toFixed(1)+')');
    var arc = theta/360*3.14*track;
    SIM.el('tcRobot').textContent = Math.round(theta);
    SIM.el('tcArc').textContent = arc.toFixed(1);
    SIM.el('tcWheel').textContent = Math.round(arc*13.04);
  }
  function sample(){
    ...
    SIM.el('tcTraceR').setAttribute('points', ptsR.join(' '));
    SIM.el('tcTraceL').setAttribute('points', ptsL.join(' '));
  }
  function showLbl(){
    var r = R()+24, a = (target - lastInc/2)*Math.PI/180;
    var t = SIM.el('tcArcLbl');
  ...
  SIM.def('tcTurn', function(deg){ SIM.el('tcArcLbl').setAttribute('opacity','0'); target += deg; lastInc = deg; animate(); });
  SIM.def('tcReset', function(){
    if(raf) cancelAnimationFrame(raf); raf = null;
    theta = 0; target = 0; ptsR = []; ptsL = [];
    SIM.el('tcTraceR').setAttribute('points','');
    SIM.el('tcTraceL').setAttribute('points','');
    SIM.el('tcArcLbl').setAttribute('opacity','0');
    draw();
  });
  SIM.def('tcTrack', function(v){
    track = parseFloat(v);
    SIM.el('tcTrackVal').textContent = track.toFixed(1);
    SIM.el('tcGap').textContent = track.toFixed(1);
    SIM.el('tcRope').textContent = (3.14*track).toFixed(1);
    SIM.el('tcQuarter').textContent = (3.14*track/4).toFixed(1);
    SIM.el('tcTA').textContent = Math.round(3.14*track/4*13.04);
    SIM.run('tcReset'); setGeom();
  });
  setGeom(); draw();
})();
</script>
...
::: {.fragment .fade-up}
::: {.cardbox .teal}
That gap has a name: the **axle track**, the wheel-to-wheel distance, **≈ 12.8 cm** on the coach's base. Wider robot → longer rope → bigger corner. **Measure your own.** Nobody copies 131.
:::
:::

::: {.notes}
This is the reveal for the last slide's question — click BEFORE explaining. Hit **Turn 90°** with their guesses on the board: both tires drag a quarter of the dashed circle, the shape question answers itself on screen, and the **¼ rope ≈ 10.0 cm** label lands right on the arc they just watched. Now the three formula lines, one fragment at a time, each reading its number off the picture: rope around the circle (3.14 × gap) — say it as yesterday's move: "rope around the *rim* gave you 27.6; same trick, bigger circle, drawn on the mat" — then a quarter of it (the labeled arc), × 13.04 → 131, next to their guesses. Don't re-teach the translator — 13.04 is yesterday's number; a quick "who remembers what 13.04 does?" is enough. Click three more times: a full robot-turn = the whole circle's rope. Save the **Turn 180°** button for the about-face slide two slides ahead — it shows ½ rope live and confirms ≈262. Then the payoff: drag the **tire gap** slider wider — every line updates live and TURN_ANGLE grows. Wider robot = more rolling per corner — *that's* why every squad measures their **own** gap (≈128 mm on our build; the stock ADB is ≈114 mm). Teal trace = right tire, coral = left, same colors as the code comments.
:::

## Unroll the formula 🧵

Your 131 is right. But every number is one of **yesterday's ropes**. Unroll them and watch π vanish:
...
<div style="font-family:Consolas,monospace;font-size:23px;color:#1C355E;background:#eef3f8;border-radius:12px;padding:16px 26px;line-height:2.1;width:fit-content;margin:0 auto;">
  <div>TURN_ANGLE&nbsp;= 3.14 × 12.8 ÷ 4&nbsp;&nbsp;× <span class="em-purple">13.04</span></div>
  <div class="fragment">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;= 3.14 × 12.8 ÷ 4&nbsp;&nbsp;× <span class="em-purple">360 ÷ (3.14 × 8.8)</span></div>
  <div class="fragment">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;= <s style="color:#E96852;">3.14</s> × 12.8 × <b>90</b> ÷ (<s style="color:#E96852;">3.14</s> × 8.8)</div>
  <div class="fragment">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;= <b>90 × 12.8 ÷ 8.8 ≈ 131°</b></div>
</div>
```
...
::: {.notes}
Act 3 of the turn-circle story: question → wheels answer → unroll the answer. Elicit the substitution, don't re-teach it — both ropes are Day-2 property: "what IS 13.04?" (360 ÷ 27.6) and "what was 27.6?" (3.14 × 8.8, the wheel's rope). The only NEW thing on this slide is the cancellation: same 3.14 on top and bottom → cancels; 360 ÷ 4 → 90. What's left is 90 × gap ÷ wheel: sanity-check on the calculator, 90 × 12.8 ÷ 8.8 = 130.9 ≈ their 131. The pocket line is a deliberate plant: **gap and wheel diameter are exactly the two numbers `DriveBase` asks for at the D8 reveal** — when that deck asks "why does it need the axle track?", call back to this cancellation. Land at least the final gap ÷ wheel line with everyone; if a squad is drowning in the algebra, the demo intuition carries the load — skim the middle steps and move on.
:::
...
```{=html}
<div style="font-family:Consolas,monospace;font-size:23px;color:#1C355E;background:#eef3f8;border-radius:12px;padding:16px 26px;line-height:2.1;width:fit-content;margin:0 auto;">
  <div>corner:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:#625D9C;font-weight:800;">90</span> × 12.8 ÷ 8.8 ≈ <b>131°</b></div>
  <div>about-face: <span style="color:#E96852;font-weight:800;">180</span> × 12.8 ÷ 8.8 ≈ <b style="color:#E96852;">262°</b></div>
</div>
```
:::
...
::: {.fragment .fade-up}
::: {.cardbox .teal}
Only the **turn** changed, from 90 to 180, so the answer roughly **doubles**. Gap and wheel are your robot's fingerprint. The turn is your order.
:::
:::

::: {.notes}
This is the payoff of the unroll: the formula *generalizes*. Elicit before revealing — most squads will say "the 90 becomes 180"; make them commit, then show the line. If anyone doubts it, flip back two slides and hit **Turn 180°** on the turn-circle demo: the label shows **½ rope ≈ 20.1 cm** and the tire counter lands on ≈262 — the picture and the formula agree. Fast squads: ask for the octagon corner (45° → 45 × 12.8 ÷ 8.8 ≈ 65°) — they'll need it after the loop slide. Every squad substitutes **their own** measured gap, not 12.8.
:::
...
TURN_ANGLE = 131    # YOUR number: ¼ turn-circle, translated
SPEED = 400         # deg/s
...
::: {.fragment .fade-up}
::: {.cardbox .coral}
**32 pasted lines**, one flipped minus sign away from a broken corner. Repeating yourself breeds bugs. Repeats are the **computer's** job.
:::
:::
...
::: {.fragment .fade-up}
::: {.cardbox .coral}
`SyntaxError`: no **colon**. Every repeat header ends in `:`, which says *"block starts here."* Fix: `for side in range(4):`
:::
:::
...
~16 lines → 8. Octagon? `range(8)` plus one new corner number: **two edits**.

Now mark your start with tape. Run it **three times**. **Where does it finish?**

::: {.notes}
Close the octagon promise fast: `range(8)` plus a 45° corner — 45 × 12.8 ÷ 8.8 ≈ 65° — two edits, and a fast squad can try it. Sides use `run_angle` — yesterday's tool — so the four sides come out even and repeatable. That's deliberate: it isolates the failure to the *turns*. And the 131 is *their* honest math from this morning, so nobody can blame the number — tires scrub and slip in a pivot, so 131 wheel-degrees ≠ 90 robot-degrees, and it varies run to run. Let them run 3× and watch the last corner miss the tape by a *different* amount each time.
:::
...
::: {.fragment .fade-up}
::: {.cardbox .coral}
The wheels really *did* roll 131°. Your math was right.
...
::: {.fragment .fade-up}
::: {.cardbox .purple}
Your **inner ear** feels *turning*. Robots have it too: the **gyro**, awake inside the hub this whole time.
:::
:::
...
<div class="sim-stage">
  ...
  <div style="text-align:left;">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-top:16px;">
      <button class="sim-btn ghost" data-sim="twistBot" data-sim-arg="-45">&#10226; Twist left 45&#176;</button>
      <button class="sim-btn ghost" data-sim="twistBot" data-sim-arg="45">&#10227; Twist right 45&#176;</button>
      <button class="sim-btn purple" data-sim="resetBot">reset_heading(0)</button>
    </div>
  </div>
</div>
<script>
(function(){
  ...
  function show(){
    SIM.el('cRobot').setAttribute('transform','rotate('+cur.toFixed(1)+')');
    SIM.el('cHdg').textContent = Math.round(cur);
  }
  function animate(){
    if(raf) cancelAnimationFrame(raf);
  ...
  SIM.def('twistBot', function(d){ target += d; animate(); });
  SIM.def('resetBot', function(){ target = 0; cur = 0; if(raf) cancelAnimationFrame(raf); raf = null; show(); });
})();
</script>
```
...
right.stop()
wait(1000)                      # let the coast settle before reading
...
::: {.fragment .fade-up}
::: {.cardbox .coral}
**Momentum.** `stop()` is instant, but a spinning robot isn't. It *coasts* past the line, like a car braking. The gyro told the truth: you **did** end at 94.
:::
:::
...
- A `while` **inside** a `for`: the count loop draws, the watch loop lands each corner.
- What's *gone*: no 131. The gyro speaks **robot-degrees**. Ask 90, it watches 90.
...
::: {.fragment .fade-up}
::: {.cardbox .purple}
Watch the *motion*, though: yesterday's counted square **glided**, and this one **lurches** to each stop. That's not the gyro, it's us flooring it and then cutting power. **Smooth ≠ right:** the smooth one *drifted*. Tomorrow: smooth **and** right. 😏
:::
:::
...
::: {.qbox}
[Each corner resets heading to 0. But heading *keeps counting*: 360, 450, 720… What if you never reset, and aim corner 1 at **90**, corner 2 at **180**, corner 3 at **270**?]{.qtitle}
...
<div style="display:flex;justify-content:center;align-items:center;gap:45px;flex-wrap:wrap;">
  ...
  <div style="text-align:left;">
    ...
    <div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:8px;max-width:290px;">
      <button class="sim-btn ghost small" data-sim="shRun" data-sim-arg="3">Triangle</button>
      <button class="sim-btn ghost small" data-sim="shRun" data-sim-arg="4">Square</button>
      <button class="sim-btn ghost small" data-sim="shRun" data-sim-arg="5">Pentagon</button>
      <button class="sim-btn ghost small" data-sim="shRun" data-sim-arg="6">Hexagon</button>
      <button class="sim-btn ghost small" data-sim="shRun" data-sim-arg="8">Octagon</button>
    </div>
  </div>
</div>
<script>
(function(){
  ...
  function addWedge(P, V, N){
    var g = SIM.el('shWedges'); g.textContent = '';
  ...
  SIM.def('shRun', function(k){
    if(raf) cancelAnimationFrame(raf);
    var n = k, v = verts(n), corner = 360/n;
    SIM.el('shN').textContent = n;
    SIM.el('shGhost').setAttribute('points', v.map(function(p){return p[0].toFixed(1)+','+p[1].toFixed(1);}).join(' '));
    SIM.el('shPath').setAttribute('points', '');
    SIM.el('shWedges').textContent = '';
    SIM.el('shArc').setAttribute('d', '');
    SIM.el('shTot').textContent = '0°';
    SIM.el('shDone').textContent = '';
    var pathPts = [v[0].slice()], edge = 0, edur = Math.max(340, 520 - n*16), t0 = performance.now();
    function place(x,y,ang){ SIM.el('shBot').setAttribute('transform','translate('+x.toFixed(1)+','+y.toFixed(1)+') rotate('+ang.toFixed(1)+')'); }
    function step(now){
      ...
      SIM.el('shPath').setAttribute('points', live.join(' '));
      if(p < 1){ raf = requestAnimationFrame(step); return; }
      pathPts.push(to.slice());
      addWedge(from, to, v[(edge+2)%n]);
      edge++;
      var done = edge*corner;
      SIM.el('shArc').setAttribute('d', arcPath(done));
      SIM.el('shTot').textContent = Math.round(done) + '°';
      if(edge < n){ t0 = performance.now(); raf = requestAnimationFrame(step); }
      else { SIM.el('shDone').textContent = 'one full lap = 360° ✓'; raf = null; }
    }
...
::: {.fragment .fade-up}
::: {.cardbox .teal}
Every corner adds up to **one full lap, 360°**, so **corner = 360 ÷ sides**.
:::
...
<!-- SECTIONS E (self-healing straight) and F (DriveBase) moved out on 2026-07-06:
     ...
     re-timed to D8 on 2026-07-15 per decision #26).
...
::: {.callout-important}
## The challenge
The judge calls a shape, triangle up to octagon. **60 seconds** to edit and run: `range(?)` plus corner = **360 ÷ sides**. Score = gap from tape to finish, tiebreak = fewest lines touched. 3 calls, best two count.
:::

- Two edits per shape. That's the whole race. Copy-pasters meet their doom at the octagon. 😈
- Pro question: which drifts more, 3 fat corners or 8 skinny ones? Watch your traces and keep the answer for tomorrow.
...
::: {.fragment .fade-up}
Tomorrow: the coast meets its cure, which is to arrive *slowly*. Then the cure gets a brain: **speed that shrinks with the error**. One formula, and it has a name every roboticist knows.
:::
