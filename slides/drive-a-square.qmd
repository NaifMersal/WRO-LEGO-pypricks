...
::: {.notes}
Everyone types this once at the top and keeps it for the whole day. Raw motors only, on purpose — today through D5 the students *build* the driver by hand; the polished shortcut (`DriveBase`) arrives on Day 5 as a reward they'll have earned piece by piece.
:::
...
```{.python code-line-numbers="|1|3-4|5-6|7-9"}
...
::: {.notes}
Everyone runs it and reports their printed number. Nobody gets exactly 90 — expect 92–96 depending on speed and battery. Don't explain yet; collect the numbers on the board and let the next slide interrogate them.
:::
...
<!-- SECTIONS E (self-healing straight) and F (DriveBase) moved out on 2026-07-06:
     ...
     quiz + loop shapes (competition: Shape Shifter); see decision log #14–15 in the plan. -->
